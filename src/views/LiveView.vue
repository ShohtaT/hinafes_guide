...
export default {
  ...
  methods: {
    loadData() {
      ...
      // データのバージョンが一致しない場合は新しいデフォルトデータを適用
      if (!storedVersion || parseInt(storedVersion) !== this.dataVersion) {
        this.categories = this.getDefaultCategories();
        this.saveData(); // 新しいデータをローカルストレージに保存
      } else if (storedCategoriesData) {
        // バージョンが一致する場合はローカルストレージからデータをロード
